...
x0, y0, z0 =map(float, line.split()[1:]) #these are in bohr

line=cube.readline()
nx=int(line.split()[0])
dx=np.array(map(float, line.split()[1:]))*bohrtoang
line=cube.readline()
ny=int(line.split()[0])
dy=np.array(map(float, line.split()[1:]))*bohrtoang
line=cube.readline()
nz=int(line.split()[0])
...
for ix in range(nx):
   for iy in range(ny):
      whole=[map(float, cube.readline().split()) for nl in range(nlines)]
